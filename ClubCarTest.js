...
function calculateSpeed(){
    ...
    speedX += accelaration*mod; 
    speedY += accelaration*mod;
    ...
    if ((x+speedX < 1600)*(x+speedX*Math.cos(angle) > 0)) {x += speedX*Math.cos(angle)};
    if ((y+speedY < 800)*(y+speedY*Math.sin(angle) > 0)) {y += speedY*Math.sin(angle)};
}
